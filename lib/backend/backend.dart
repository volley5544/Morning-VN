...
/// Functions to query ToolsMenuRecords (as a Stream and as a Future).
Future<int> queryToolsMenuRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
...
Stream<List<ToolsMenuRecord>> queryToolsMenuRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future<List<ToolsMenuRecord>> queryToolsMenuRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future maybeCreateUser(User user) async {
  final userRecord = UserLocationLogRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument =
        await UserLocationLogRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createUserLocationLogRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      UserLocationLogRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createUserLocationLogRecordData(email: email));
}
